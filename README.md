...
6. **API da IA** — `firebase functions:secrets:set ANTHROPIC_API_KEY`. Em `functions/.env.ofina-6119d` defina `BASE_URL=https://ofina-6119d.web.app` e, se quiser, `AI_MODEL`. Sem a chave, o assistente responde em modo demonstração.
...
## Atualização
`firebase-config.js` já está preenchido com o projeto `ofina-6119d` (use `firebase use ofina-6119d`). O app ainda não lê nem grava no Firebase: ainda usa dados locais. O PDF do orçamento é gerado no próprio app e enviado pela tela de compartilhar do celular.
